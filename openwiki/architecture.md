...
`pdb`, `hpa`, `spread`, `probes`, `qos`, `svc-backends`, `pending` share a shape
(see [`internal/view/pdb.go`](../internal/view/pdb.go) as the reference):
...
`spread`, `probes`, `qos`, and `svc-backends` reuse them (`pending` renders a
plain `REASON` column and only needs `SortBy`).
