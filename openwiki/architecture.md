...
`pdb`, `hpa`, `spread`, `probes`, `qos`, `svc-backends`, `rollouts` and
`pending` share a shape (see
...
`spread`, `probes`, `qos`, `svc-backends`, and `rollouts` reuse them (`pending`
renders a plain `REASON` column and only needs `SortBy`).
